...
class CheckIn extends Component {
	...
	handleSubmit() {
		const { goalId, date, weight, reps, sets, min, secs } = this.state;
	...
	renderTableRow() {
		const { today, goal, checkins } = this.state;
						...
						<Table.Row key={checkin._id}>
							...
							<td>
								<input
									type="button"
									onClick={() => {
										this.handleRemoveCheckIn(checkin._id);
									}}
									value="&times;"
								/>
							</td>
	...
	render() {
		...
		return (
			<div>
				{console.log(
					"state of the goals in goal component",
					this.props.location.state.goal
				)}
				{console.log("checkin whats in checkins", checkins)}
				<MenuBar />
